...
export default function AdminDashboard() {
  ...
  /* -------- Enrollment modal -------- */
  const [enrollCourse, setEnrollCourse] = useState(null); // course object or null
  ...
  async function refreshStudents({ q = sQ } = {}) {
    ...
    try {
      const res = q?.trim()
        ...
        : await listStudents({ all: true, pageSize: 200 });
      setSData(shape(res));
      setSQ(q);
    } catch (err) {
  ...
  async function refreshCourses({ q = cQ } = {}) {
    ...
    try {
      const res = q?.trim()
        ...
        : await listCourses({ all: true, pageSize: 200 });
      setCData(shape(res));
      setCQ(q);
    } catch (err) {
  ...
  async function refreshTutors({ q = tQ } = {}) {
    ...
    try {
      const res = q?.trim()
        ...
        : await listTutors({ all: true, pageSize: 200 });
      setTData(shape(res));
      setTQ(q);
    } catch (err) {
  ...
  async function onCreateStudent(e) {
    ...
    try {
      ...
      alert(err?.response?.data?.message || err?.message || "Could not create student");
    } finally {
      setSBusy(false);
    }
  }
  ...
  async function onUpdateStudent(e) {
    ...
    setSBusy(true);
    try {
      await updateStudent(sOpenEdit._id || sOpenEdit.id, payload);
      setSOpenEdit(null);
      await refreshStudents();
    } catch (err) {
      alert(err?.response?.data?.message || err?.message || "Could not update student");
    } finally {
      setSBusy(false);
    }
  }
  ...
  async function onDeleteStudent(id) {
    ...
    try {
      ...
      alert(err?.response?.data?.message || err?.message || "Could not delete student");
    }
  }
  ...
  async function onCreateCourse(e) {
    ...
    try {
      ...
      alert(err?.response?.data?.message || err?.message || "Could not create course");
    } finally {
      setCBusy(false);
    }
  }
  ...
  async function onUpdateCourse(e) {
    ...
    setCBusy(true);
    try {
      await updateCourse(cOpenEdit._id || cOpenEdit.id, payload);
      setCOpenEdit(null);
      await refreshCourses();
    } catch (err) {
      alert(err?.response?.data?.message || err?.message || "Could not update course");
    } finally {
      setCBusy(false);
    }
  }
  ...
  async function onDeleteCourse(id) {
    ...
    try {
      ...
      alert(err?.response?.data?.message || err?.message || "Could not delete course");
    }
  }
  ...
  async function openEnrollModal(course) {
    ...
    setEnrollStudentId("");
    if (!sData.items.length) await refreshStudents({ q: "" });
  }
  ...
  async function submitEnroll(e) {
    ...
    setEnrollBusy(true);
    try {
      await enrollStudentInCourse(enrollCourse._id || enrollCourse.id, enrollStudentId);
      setEnrollCourse(null);
      setEnrollStudentId("");
      await refreshCourses();
    } catch (err) {
      alert(err?.response?.data?.message || err?.message || "Could not enroll student");
    } finally {
      setEnrollBusy(false);
    }
  }
  ...
  async function onCreateTutor(e) {
    ...
    try {
      ...
      alert(err?.response?.data?.message || err?.message || "Could not create tutor");
    } finally {
      setTBusy(false);
    }
  }
  ...
  async function onUpdateTutor(e) {
    ...
    setTBusy(true);
    try {
      await updateTutor(tOpenEdit._id || tOpenEdit.id, payload);
      setTOpenEdit(null);
      await refreshTutors();
    } catch (err) {
      alert(err?.response?.data?.message || err?.message || "Could not update tutor");
    } finally {
      setTBusy(false);
    }
  }
  ...
  async function onDeleteTutor(id) {
    ...
    try {
      ...
      alert(err?.response?.data?.message || err?.message || "Could not delete tutor");
    }
  }
  ...
  return (
    <div className="min-h-screen bg-cream">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {activeView === "students" && (
          <StudentsSection
            ...
            q={sQ}
            setQ={setSQ}
            onSearch={() => refreshStudents({ q: sQ })}
            openNew={() => setSOpenNew(true)}
            setOpenEdit={setSOpenEdit}
            loading={showStudentsLoader}
        ...
        {activeView === "courses" && (
          <CoursesSection
            ...
            q={cQ}
            setQ={setCQ}
            onSearch={() => refreshCourses({ q: cQ })}
            openNew={() => setCOpenNew(true)}
            setOpenEdit={setCOpenEdit}
            loading={showCoursesLoader}
        ...
        {activeView === "tutors" && (
          <TutorsSection
            ...
            q={tQ}
            setQ={setTQ}
            onSearch={() => refreshTutors({ q: tQ })}
            openNew={() => setTOpenNew(true)}
            setOpenEdit={setTOpenEdit}
            loading={showTutorsLoader}
      ...
      {/* Student Modals */}
      <StudentCreateModal open={sOpenNew} busy={sBusy} onClose={() => setSOpenNew(false)} onSubmit={onCreateStudent} />
      <StudentEditModal open={!!sOpenEdit} busy={sBusy} onClose={() => setSOpenEdit(null)} onSubmit={onUpdateStudent} data={sOpenEdit} />

      {/* Course Modals */}
      <CourseCreateModal open={cOpenNew} busy={cBusy} onClose={() => setCOpenNew(false)} onSubmit={onCreateCourse} />
      <CourseEditModal open={!!cOpenEdit} busy={cBusy} onClose={() => setCOpenEdit(null)} onSubmit={onUpdateCourse} data={cOpenEdit} />

      {/* Tutor Modals */}
      <TutorCreateModal open={tOpenNew} busy={tBusy} onClose={() => setTOpenNew(false)} onSubmit={onCreateTutor} />
      <TutorEditModal open={!!tOpenEdit} busy={tBusy} onClose={() => setTOpenEdit(null)} onSubmit={onUpdateTutor} data={tOpenEdit} />

      {/* ENROLL MODAL */}
      <Modal
        open={!!enrollCourse}
        onClose={() => setEnrollCourse(null)}
        title={`Enroll student in ${enrollCourse?.code ? `${enrollCourse.code} – ` : ""}${enrollCourse?.title || "Course"}`}
        footer={
          <>
            <button
            ...
            <button
              ...
              disabled={enrollBusy || !enrollStudentId}
            >
      ...
      >
        {enrollCourse && (
          <form id="enrollStudent" onSubmit={submitEnroll} className="space-y-4">
            <label className="block">
              <span className="text-sm font-heading font-medium text-primary">Select Student *</span>
              <select
                required
                value={enrollStudentId}
                onChange={(e) => setEnrollStudentId(e.target.value)}
                className="mt-1 w-full rounded-lg border-2 border-primary/30 px-3 py-2 font-sans focus:ring-2 focus:ring-accent focus:border-accent"
              >
            ...
            <p className="text-xs text-primary-800/70">
              Tip: If you don’t see a student, open the <b>Students</b> tab and add/search first.
            </p>
          </form>
        )}
      </Modal>
...
function Overview({ stats, systemAlerts, recentActivity, upcomingTasks }) {
  return (
                ...
                >
                  <div className="flex items-start gap-3 justify-between">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="font-heading font-medium text-primary">{a.message}</p>
                        <span
...
function StudentsSection({ data, q, setQ, onSearch, openNew, setOpenEdit, loading, onDelete }) {
  return (
...
function CoursesSection({ data, q, setQ, onSearch, openNew, setOpenEdit, loading, onDelete, onEnroll }) {
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <h2 className="text-2xl font-heading font-bold text-primary">Course Management</h2>
          <p className="text-primary-800 font-sans text-sm">Total: {data.total} courses</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex gap-2">
            <input
            ...
            <table className="w-full text-sm font-sans">
              <thead>
                <tr className="border-b-2 border-primary/20 bg-lavender/20">
                  ...
                  <th className="py-4 px-4 text-left font-heading font-semibold text-primary w-[260px]">Actions</th>
                </tr>
              </thead>
              <tbody>
                {data.items.map((c) => (
                  <tr key={c._id || c.id} className="border-b border-primary/10 hover:bg-lavender/5 transition">
                    ...
                    <td className="py-3 px-4">
                      <div className="flex gap-2">
                        ...
                        <button
                          className="p-2 rounded-lg border-2 border-accent/60 hover:bg-accent/20 transition text-primary flex items-center gap-1"
                          onClick={() => onEnroll(c)}
                        >
                          <UserPlus className="w-4 h-4" />
                          <span className="text-xs font-button">Enroll Student</span>
                        </button>
                      </div>
                    </td>
                  </tr>
...
function TutorsSection({ data, q, setQ, onSearch, openNew, setOpenEdit, loading, onDelete }) {
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <h2 className="text-2xl font-heading font-bold text-primary">Tutor Management</h2>
          <p className="text-primary-800 font-sans text-sm">Total: {data.total} tutors</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex gap-2">
            <input
...
function StudentCreateModal({ open, onClose, onSubmit, busy }) {
  return (
    <Modal
      open={open}
      onClose={onClose}
    ...
    >
      <form id="createStudent" onSubmit={onSubmit} className="space-y-4">
        <label className="block">
...
function StudentEditModal({ open, onClose, onSubmit, busy, data }) {
  return (
    <Modal
      open={open}
      onClose={onClose}
    ...
    >
      {data && (
        <form id="editStudent" onSubmit={onSubmit} className="space-y-4">
          <label className="block">
            <span className="text-sm font-heading font-medium text-primary">Name *</span>
            <input
              name="name"
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <label className="block">
              ...
              <input
                name="phone"
                defaultValue={data.phone ?? data?.emergencyContact?.phone ?? ""}
                className="mt-1 w-full rounded-lg border-2 border-primary/30 px-4 py-2 font-sans focus:ring-2 focus:ring-accent focus:border-accent"
              />
            </label>
          </div>
...
function CourseCreateModal({ open, onClose, onSubmit, busy }) {
  return (
    <Modal
      open={open}
      onClose={onClose}
    ...
    >
      <form id="createCourse" onSubmit={onSubmit} className="space-y-4">
...
function CourseEditModal({ open, onClose, onSubmit, busy, data }) {
  return (
    <Modal
      open={open}
      onClose={onClose}
    ...
    >
      {data && (
        <form id="editCourse" onSubmit={onSubmit} className="space-y-4">
          <div className="grid sm:grid-cols-2 gap-4">
            <label className="block">
              <span className="text-sm font-heading font-medium text-primary">Code</span>
              <input
...
function TutorCreateModal({ open, onClose, onSubmit, busy }) {
  return (
    <Modal
      open={open}
      onClose={onClose}
    ...
    >
      <form id="createTutor" onSubmit={onSubmit} className="space-y-4">
        <div className="grid sm:grid-cols-2 gap-4">
          <label className="block">
            <span className="text-sm font-heading font-medium text-primary">Name *</span>
            <input
...
function TutorEditModal({ open, onClose, onSubmit, busy, data }) {
  return (
    <Modal
      open={open}
      onClose={onClose}
    ...
    >
      {data && (
        <form id="editTutor" onSubmit={onSubmit} className="space-y-4">
          <div className="grid sm:grid-cols-2 gap-4">
            <label className="block">
              <span className="text-sm font-heading font-medium text-primary">Name</span>
              <input
