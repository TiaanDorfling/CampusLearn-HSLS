...
function getChatInstance(): Chat {
  ...
  if (!apiKey) throw new Error("VITE_GEMINI_API_KEY environment variable not set");
...
type BotResponse = Omit<Message, 'sender'>; // Assumes 'Message' has 'text' and 'suggestedReplies'
...
async function parseBotResponse(response: GenerateContentResponse): Promise<BotResponse> {
  ...
  if (fullText.startsWith('```json') && fullText.endsWith('```')) {
    // Remove '```json\n' from the start and '\n```' from the end
    fullText = fullText.substring('```json'.length, fullText.length - '```'.length).trim();
  ...
  try {
    ...
    console.warn("Failed to parse Gemini response as JSON (even after markdown strip). Falling back to plain text.", e);
